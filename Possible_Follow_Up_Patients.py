...
class PossibleFollowUpPatientsView(ctk.CTkFrame):
    ...
    def show_main_menu(self, patient_id, name, surname):
        ...
        # Insert in follow up button
        followup_button = ctk.CTkButton(
            actions_frame,
            text="Insert in Follow Up",
            width=330,
            height=50,
            fg_color="#73C8AE",
            hover_color="#046A38",
            font=("Arial", 16),
            text_color="white",
            command=lambda: self.insert_in_follow_up(patient_id, name, surname)
        )
        followup_button.pack(pady=15)
    ...
    def insert_in_follow_up(self, patient_id, name, surname):
        conn = sqlite3.connect("Database_proj.db")
        cursor = conn.cursor()
        
        try:
            # Get patient data from Possible_Follow_Up_Patients
            cursor.execute("""
                SELECT PatientID, Name, Surname, Days_since_last_OSA
            ...
            if patient_data:
                ...
                cursor.execute("""
                    INSERT INTO Follow_Up_Patients (PatientID, Name, Surname, Date)
                    VALUES (?, ?, ?, date('now'))
                """, (patient_data[0], patient_data[1], patient_data[2]))
                
                # Delete from Possible_Follow_Up_Patients
                cursor.execute("""
                    DELETE FROM Possible_Follow_Up_Patients
                    WHERE PatientID = ?
                """, (patient_id,))
                ...
                # Disable the button
                for widget in self.main_frame.winfo_children():
                    if isinstance(widget, ctk.CTkButton) and widget.cget("text") == "Insert in Follow Up":
                        widget.configure(
                            state="disabled",
                            fg_color="grey",
                            hover_color="grey"
                ...
                success_label = ctk.CTkLabel(
                    self.main_frame,
                    text="Patient moved to Follow Up successfully!",
                    font=("Arial", 16),
                    text_color="#046A38"
                )
                success_label.pack(pady=20)
                ...
                self.options_window.after(2000, self.options_window.destroy)
                
        except sqlite3.Error as e:
            error_label = ctk.CTkLabel(
                self.main_frame,
                text=f"Error moving patient: {str(e)}",
                text_color="red"
            )
            error_label.pack(pady=20)
        finally:
